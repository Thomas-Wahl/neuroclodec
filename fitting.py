...
def complex_indices(a: numpy.ndarray):
...
def starting_poles(s, n: int=None, dt=None, log_space=False):
    n_real = 0 if n is None else n % 2
    n_pair = (len(s) if n is None else n) // 2
    n = n_real + n_pair
    omega = numpy.abs(s.imag)
    omega[omega == 0.] += 10.**-n
    if log_space:
        beta = numpy.logspace(numpy.log10(min(omega)), numpy.log10(max(omega)), n)
    else:
        beta = numpy.linspace(min(omega), max(omega), n)
    alpha = beta / 100.
    poles = numpy.zeros(n_real + 2 * n_pair, complex)
    if n_real:
        poles[0] = alpha[0]
    for i in range(n_real, n):
        poles[2 * i     - n_real] = alpha[i] - 1j * beta[i]
        poles[2 * i + 1 - n_real] = alpha[i] + 1j * beta[i]
...
def real_lti(a, C, D, E, improper=False, dt=None):
    A, B, C = real_ss(a, C)
    ...
    if improper:
        if len(E) > 1:
            ...
            sE = (control.TransferFunction.z if dt else control.TransferFunction.s) * E
        sys = control.xferfcn._convert_to_transfer_function(control.StateSpace(A, B, C, D, dt)) + sE
    else:
...
def sort_residues(a: numpy.ndarray, p: numpy.ndarray, r: numpy.ndarray):
...
def sigma_zeros(a: numpy.ndarray, C: numpy.ndarray, D: float, discrete=False, stable=True):
    A, B, C = real_ss(a, C)
...
def sigma_symmetric_zeros(a: numpy.ndarray, C: numpy.ndarray, D: float):
    ...
    eigvals -= 2. * eigvals.real * numpy.logical_and(eigvals.imag == 0., eigvals.real < 0.)
    # take the square root of the eigvals and only select the ones with a negative real part
    eigvals **= .5
    eigvals -= 2. * eigvals.real * (eigvals.real > 0.)
    # sort poles so that the real ones come first
    return sort_eigvals(eigvals)
...
def solve_poles(
    ...
    if relax:
        ...
        for m in range(Nc):
            ...
            A[m * (N + 1): (m + 1) * (N + 1)] = R22
            if m + 1 == Nc:
                b[m * (N + 1): (m + 1) * (N + 1)] = scale * Q[-1, offset:].conj() * Ns
    ...
    return sigma_symmetric_zeros(a, Ct, Dt) if symmetric else sigma_zeros(a, Ct, Dt, discrete, stable)
...
def check_params(s, f, poles, weights, strictly_proper, improper, dt, relax):
    ...
    if strictly_proper and improper:
        raise ValueError('LTI system cannot be both strictly proper and improper.')
    ...
    if poles is None:
        n = (len(s) - ((not strictly_proper) + improper + (f.shape[0] > 1 or relax)) / 2).__floor__()
        poles = starting_poles(s, n, dt)
    elif isinstance(poles, int):
        poles = starting_poles(s, poles, dt)
    else:
        # check dimensions
        poles = numpy.atleast_1d(poles)
        if poles.ndim > 1:
            raise ValueError(f'poles must be 1-dimensional: {poles.ndim = }')
    if True:
        # check number of sample points
        min_Ns = (len(poles) + ((not strictly_proper) + improper + (f.shape[0] > 1 or relax)) / 2).__ceil__()
        if len(s) < min_Ns:
            raise ValueError(f's length must be at least {min_Ns}: current length = {len(s)}')
    # set default values
    weights = numpy.ones(f.shape) if weights is None else numpy.atleast_1d(weights)
    # check dimensions
    if weights.ndim > 2:
        raise ValueError(f'weights must be at most 2-dimensional: {weights.ndim = }')

    # force weights to be an Nc×Ns array
    if weights.ndim == 1:
        if len(weights) not in f.shape:
            raise ValueError(f'mismatch between weights length and f dimensions: {len(weights) = }, {f.shape = }')
        ...
        if sorted(weights.shape) != sorted(f.shape):
            raise ValueError(f'mismatch between weights dimensions and f dimensions: {weights.shape = }, {f.shape = }')
...
def vectfit(
    ...
    s, f, a, w, strictly_proper, improper = check_params(s, f, poles, weights, strictly_proper, improper, dt, relax)
    
    for _ in range(niter):
        a = solve_poles(s, f, a, w, strictly_proper, improper, bool(dt), stable, relax)
...
def magvectfit(
    ...
    s, f, a, w, strictly_proper, improper = check_params(s, f, poles, weights, strictly_proper, improper, None, relax)
    # magVF specific checks
    f = f.astype(float)
    if (f < 0.).any():
        raise ValueError('f must contain only positive real values.')
    
    for _ in range(niter):
        a = solve_poles(s, f, a, w, strictly_proper, improper, False, False, relax, symmetric=True)
    
    C, D, E = solve_residues(s, f, a, w, strictly_proper, improper, symmetric=True)

    return real_lti(a, C, D, E, improper)
